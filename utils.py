...
def get_data(data_path: str, data_type: str):
    ...
    """
    data = [pd.read_parquet(file, engine='auto') for file in glob(f"{data_path}/*{data_type}")]
...
def clean_and_parse_url(url: str) -> dict:
    ...
    # Decode the url
    # Ex : dictionnaire.reverso.net/francais-synonymes/d%C3%A9faut+de+qualit%C3%A9" will become
    # dictionnaire.reverso.net/francais-synonymes/défaut+de+qualité
    ...
    groups = re.match(pattern, url_decoded).groupdict()
